---
name: editos-install
description: Install, configure, and start FireRed-EditOS from source on a local machine. Use when a user asks to set up EditOS, troubleshoot installation, download required resources, fill config.toml API keys, or launch the MCP and web services, as well as Chinese requests like “安装 EditOS”, “配置 EditOS”, “启动 EditOS”, “把 EditOS 跑起来”, “修复 EditOS 安装问题”, or “排查 EditOS 启动失败”.
---

# EditOS Install

Use this skill when the task is to install or repair a local source checkout of FireRed-EditOS.
...
- Download `.editos` models and `resource/` assets
...
```bash
conda create -y -n editos-py311 python=3.11
conda run -n editos-py311 python --version
conda run -n editos-py311 python -m venv .venv
```
...
```bash
.venv/bin/pip check
PYTHONPATH=src .venv/bin/python -c "from edit_os.config import load_settings; load_settings('config.toml'); print('config_ok')"
```

Also confirm key resources exist:

```bash
test -f .editos/models/transnetv2-pytorch-weights.pth
...
```bash
PYTHONPATH=src .venv/bin/python -m edit_os.mcp.server
```
