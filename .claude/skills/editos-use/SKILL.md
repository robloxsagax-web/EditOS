---
name: editos-use
description: Use this skill when EditOS is already installed and the user wants to start the local MCP/Web services, create or continue a session, send editing instructions, perform multi-turn re-editing, and verify rendered video outputs, as well as Chinese requests like “启动 EditOS”, “把 EditOS 跑起来”, “用 EditOS 剪视频”.
---

# EditOS Usage Skill

你负责在“已安装完成”的前提下，执行 EditOS 的实际剪辑流程。
EditOS 是一个剪辑 Agent，用户可使用自己的素材，通过自然语言对话的方式剪辑视频。内置素材搜索、内容理解、生成字幕、文字转语音等功能，用户可以多次提出具体的剪辑/修改意见。
...
不处理完整安装流程（依赖安装、模型下载、资源下载等），那是安装技能的范围。如果在启动时遇到问题，怀疑是安装问题，再去查看安装Skill editos-install
...
2. 优先复用现有脚本，不要重复造轮子：
   ...
   - Web 服务桥接脚本位于当前 skill 目录下的 `scripts/bridge_editos.py`。请先定位当前 skill 目录，再拼接`scripts/bridge_editos.py`  
...
后续命令中的 `<repo-root>` 指向 EditOS 仓库根目录，例如：

```bash
/Users/yourname/Desktop/code/Ope.editos/FireRed-Ope.editos
```
...
macOS/Linux:
```bash
cd <repo-root> && source .venv/bin/activate && PYTHONPATH=src python -m edit_os.mcp.server
```
...
```bash
cd <repo-root> && source .venv/bin/activate && python <skills-root>/scripts/bridge_editos.py \
...
```bash
cd <repo-root> && find .editos/.server_cache/<session_id> -name "output_*.mp4" 2>/dev/null
```
