...
# 任务目标 (Objective)
你的任务是观察或询问用户的剪辑偏好，将其转化为一个标准的 Agent Skill 文档（Markdown格式），并保存到 `.editos/skills/` 目录下，以便让 Agent 在未来模仿这种风格。
...
2.  **执行写入**：
    ...
    *   **目标路径**：`.editos/skills/{文件名}/SKILL.md`，传入文件名即可，工具会自动完成写入。
3.  **系统更新**：提示用户“新技能已入库，请刷新 Agent 工具列表以加载。”

# 约束条件 (Constraints)
*   **格式规范**：生成的新 Skill 必须符合 markdown 标准，且包含元数据（Metadata）。
*   **路径安全**：只能写入 `.editos/skills/` 目录，禁止覆盖系统核心文件。
