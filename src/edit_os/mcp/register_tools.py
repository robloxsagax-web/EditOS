...
from edit_os.skills.skills_io import dump_skills
from edit_os.utils.register import NODE_REGISTRY

from edit_os.mcp.sampling_requester import make_llm
from edit_os.nodes.core_nodes.base_node import BaseNode
from edit_os.nodes.node_summary import NodeSummary
from edit_os.nodes.node_state import NodeState
from src.edit_os.storage.agent_memory import ArtifactStore
...
def register(server: FastMCP, cfg: Settings) -> None:
    ...
    async def mcp_read_history(
        ...
        try:
            store = ArtifactStore(artifacts_dir=params.get('artifacts_dir', ".editos/.server_cache"), session_id=session_id)
    ...
    async def mcp_write_skills(
        ...
        skill_dir: Annotated[str, Field(description="Skill storage directory, defaults to '.editos/skills/'")] = '.editos/skills/',
