...
import numpy as np
from edit_os.utils.logging import MCPMoviePyLogger
from PIL import Image, ImageDraw, ImageFont, ImageOps
from edit_os.utils.register import NODE_REGISTRY
...
from src.edit_os.config import Settings
from edit_os.nodes.core_nodes.base_node import BaseNode, NodeMeta
from edit_os.nodes.node_state import NodeState
from edit_os.nodes.node_schema import RenderVideoInput
from edit_os.utils.util import get_video_rotation
...
class RenderVideoPipeline:
    ...
    @staticmethod
    def _apply_native_transition(clip1: VideoClip, clip2: VideoClip, transition_type: str = "crossfade", duration: float = 0.4) -> VideoClip:
        """
        Apply a native MoviePy transition between two clips.
        
        Args:
            clip1: First clip (end portion will fade out)
            clip2: Second clip (start portion will fade in)
            transition_type: Type of transition
            duration: Duration of transition in seconds
        
        Returns:
            Composite clip with transition applied
        """
        # Calculate overlap regions
        overlap_s = min(duration, clip1.duration * 0.5, clip2.duration * 0.5)
        
        if transition_type == "crossfade":
            # Crossfade: clip1 fades out while clip2 fades in
            clip1_end = clip1.subclipped(clip1.duration - overlap_s, clip1.duration)
            clip2_start = clip2.subclipped(0, overlap_s)
            
            clip1_faded = clip1_end.with_effects([vfx.FadeOut(overlap_s)])
            clip2_faded = clip2_start.with_effects([vfx.FadeIn(overlap_s)])
            
            # Concatenate: clip1 (non-overlap), composite overlap, clip2 (non-overlap)
            clip1_main = clip1.subclipped(0, clip1.duration - overlap_s)
            clip2_main = clip2.subclipped(overlap_s, clip2.duration)
            
            return concatenate_videoclips([clip1_main, CompositeVideoClip([clip1_faded, clip2_faded]), clip2_main], method="compose")
        
        elif transition_type == "zoom_punch":
            # Zoom punch: rapid zoom effect that hides cut point
            zoom_factor = 1.0 + (0.3 * duration)
            clip1_end = clip1.subclipped(clip1.duration - overlap_s, clip1.duration)
            clip2_start = clip2.subclipped(0, overlap_s)
            
            # Apply zoom to clip1 ending and fade
            clip1_zoomed = clip1_end.with_effects([vfx.ZoomIn(overlap_s, zoom_factor), vfx.FadeOut(overlap_s * 0.5)])
            clip2_faded = clip2_start.with_effects([vfx.FadeIn(overlap_s * 0.5)])
            
            clip1_main = clip1.subclipped(0, clip1.duration - overlap_s)
            clip2_main = clip2.subclipped(overlap_s, clip2.duration)
            
            return concatenate_videoclips([clip1_main, CompositeVideoClip([clip1_zoomed, clip2_faded]), clip2_main], method="compose")
            ...
            flash_clip = ColorClip(size=clip1.size, color=(255, 255, 255)).with_duration(overlap_s * 0.3).with_effects([vfx.FadeOut(overlap_s * 0.3)])
            
            clip1_main = clip1.subclipped(0, clip1.duration - overlap_s)
            clip2_main = clip2.subclipped(overlap_s, clip2.duration)
            
            return concatenate_videoclips([clip1_main, CompositeVideoClip([clip1_end, flash_clip]), clip2_main], method="compose")
        
        elif transition_type in ("slide_left", "slide_right"):
            # Slide transition
            direction = "left" if transition_type == "slide_left" else "right"
            clip2_sliding = clip2.with_effects([vfx.slide_in(overlap_s, direction)])
            
            clip1_end = clip1.subclipped(clip1.duration - overlap_s, clip1.duration).with_effects([vfx.slide_out(overlap_s, direction)])
            
            clip1_main = clip1.subclipped(0, clip1.duration - overlap_s)
            clip2_main = clip2.subclipped(overlap_s, clip2.duration)
            
            return concatenate_videoclips([clip1_main, CompositeVideoClip([clip1_end, clip2_sliding]), clip2_main], method="compose")
        
        else:
            # Default: simple crossfade
            return RenderVideoPipeline._apply_native_transition(clip1, clip2, "crossfade", duration)

    @staticmethod
    def _get_transition_clip(clip: VideoClip, transition_type="fade_in", transition_duration=1.0):
        """
        Apply transition effects to a clip (for opening/ending).
        
        Available native transitions (FREE - no API needed):
        - fade_in: Fade in from black
        - fade_out: Fade out to black  
        - crossfade: Smooth blend between clips
        - zoom_punch: Rapid zoom in (1.1x → 1.3x) for motion blur effect
        - flash: Quick white flash transition
        - slide_left: Slide transition from right
        - slide_right: Slide transition from left
        ...
        all_transition = {
            ...
            # Native transitions
            "crossfade": clip.with_effects([vfx.CrossFadeIn(transition_duration)]),
            "zoom_punch": clip.with_effects([
                vfx.ZoomIn(transition_duration, 1.0),
                vfx.SpeedUp(1.0 + (0.3 * transition_duration))
            ]),
            "flash": clip.with_effects([vfx.FadeIn(transition_duration * 0.3)]),
            "slide_left": clip.with_effects([vfx.slide_in(transition_duration, "left")]),
            "slide_right": clip.with_effects([vfx.slide_in(transition_duration, "right")]),
        }
